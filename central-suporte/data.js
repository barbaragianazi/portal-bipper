...
const DEFAULT_ENVIRONMENTS = [
  { id: 'ENV-1', company: 'Zoetis', platform: 'BIPPER', instance: 'zoetis-ru-prod', label: 'Zoetis Ruminantes', database: 'db_zoetis_ru', region: 'São Paulo', health: 'online', color: '#F65C00', image: '', apiUrl: '', defaultSla: 8, supportUsers: [], notes: '' },
  { id: 'ENV-2', company: 'Zoetis', platform: 'BIPPER', instance: 'zoetis-ca', label: 'Zoetis Animais e CIA', database: 'db_zoetis_ca', region: 'Miami', health: 'online', color: '#F65C00', image: '', apiUrl: '', defaultSla: 8, supportUsers: [], notes: '' },
  { id: 'ENV-3', company: 'BASF', platform: 'BIPPER', instance: 'basf-br-prod', label: 'BASF Brasil', database: 'db_basf_prod', region: 'São Paulo', health: 'warning', color: '#65AC1E', image: '', apiUrl: '', defaultSla: 8, supportUsers: [], notes: '' },
  { id: 'ENV-4', company: 'BASF', platform: 'BIPPER', instance: 'basf-campanhas', label: 'BASF Campanhas', database: 'db_basf_campaign', region: 'Virgínia', health: 'online', color: '#65AC1E', image: '', apiUrl: '', defaultSla: 8, supportUsers: [], notes: '' },
  { id: 'ENV-5', company: 'Elanco', platform: 'BIPPER', instance: 'elanco-br-prod', label: 'Elanco Brasil', database: 'db_elanco_br', region: 'São Paulo', health: 'online', color: '#0072CE', image: '', apiUrl: '', defaultSla: 8, supportUsers: [], notes: '' },
  { id: 'ENV-6', company: 'Syngenta', platform: 'BIPPER', instance: 'syngenta-training', label: 'Syngenta Treinamentos', database: 'db_syngenta_training', region: 'Frankfurt', health: 'online', color: '#5F7800', image: '', apiUrl: '', defaultSla: 8, supportUsers: [], notes: '' },
  { id: 'ENV-7', company: 'MasterCard', platform: 'Coaliza', instance: 'mastercard-br-prod', label: 'MasterCard Brasil', database: 'db_mastercard_prod', region: 'São Paulo', health: 'online', color: '#EB001B', image: '', apiUrl: '', defaultSla: 8, supportUsers: [], notes: '' }
];
const ENVIRONMENTS_STORAGE_KEY = 'supportPrototypeEnvironmentsV2';
function normalizeEnvironment(e) { return { ...e, image: e.image || '', apiUrl: e.apiUrl || '', defaultSla: Number(e.defaultSla || 8), supportUsers: Array.isArray(e.supportUsers) ? e.supportUsers : [], notes: e.notes || '' } }
function loadEnvironments() { try { const data = JSON.parse(localStorage.getItem(ENVIRONMENTS_STORAGE_KEY)) || clone(DEFAULT_ENVIRONMENTS); return data.map(normalizeEnvironment) } catch { return clone(DEFAULT_ENVIRONMENTS).map(normalizeEnvironment) } }
function saveEnvironments() { try { localStorage.setItem(ENVIRONMENTS_STORAGE_KEY, JSON.stringify(ENVIRONMENTS)) } catch { /* armazenamento indisponível; mantém dados durante a sessão */ } }
let ENVIRONMENTS = loadEnvironments();
const INITIAL_TICKETS = [
  { id: 'TK-2081', company: 'Zoetis', platform: 'BIPPER', instance: 'zoetis-ru-prod', requester: 'Camila Ferreira', subject: 'Plano de ação não salva responsáveis', module: 'Plano de ação', priority: 'Alta', status: 'Em andamento', assignee: 'Rafael Lima', createdAt: '2026-08-03T10:15:00-03:00', updatedAt: '2026-08-03T18:42:00-03:00', slaHours: 4, slaState: 'risk', unreadSupport: 2, description: 'Ao concluir o cadastro do plano de ação, os responsáveis selecionados deixam de aparecer após atualizar a página.', attachments: ['captura-plano-acao.png'], history: [{ text: 'Ticket criado por Camila Ferreira', time: 'Hoje, 10:15' }, { text: 'Atribuído para Rafael Lima', time: 'Hoje, 10:24' }, { text: 'Análise iniciada na instância zoetis-ru-prod', time: 'Hoje, 11:02' }] },
  { id: 'TK-2080', company: 'BASF', platform: 'BIPPER', instance: 'basf-br-prod', requester: 'João Mendes', subject: 'Campanha regional não liberada', module: 'Campanha', priority: 'Média', status: 'Aberto', assignee: 'Não atribuído', createdAt: '2026-08-03T15:08:00-03:00', updatedAt: '2026-08-03T15:08:00-03:00', slaHours: 8, slaState: 'ok', description: 'A campanha foi publicada, mas não está disponível para os usuários da regional Sul.', attachments: [], history: [{ text: 'Ticket criado por João Mendes', time: 'Hoje, 15:08' }] },
  { id: 'TK-2079', company: 'Elanco', platform: 'BIPPER', instance: 'elanco-br-prod', requester: 'Patrícia Moura', subject: 'Fechamento mensal com valores divergentes', module: 'Fechamento', priority: 'Crítica', status: 'Em andamento', assignee: 'Bruno Tavares', createdAt: '2026-08-03T08:25:00-03:00', updatedAt: '2026-08-03T17:10:00-03:00', slaHours: 1, slaState: 'breached', unreadSupport: 1, description: 'O fechamento mensal apresenta total divergente do relatório financeiro e impede a conclusão do processo.', attachments: ['fechamento-julho.xlsx', 'log-fechamento.txt'], history: [{ text: 'Ticket crítico criado por Patrícia Moura', time: 'Hoje, 08:25' }, { text: 'Atribuído para Bruno Tavares', time: 'Hoje, 08:31' }, { text: 'SLA de primeira resposta ultrapassado', time: 'Hoje, 09:26' }] },
  { id: 'TK-2078', company: 'Zoetis', platform: 'BIPPER', instance: 'zoetis-ca', requester: 'Camila Ferreira', subject: 'Pagamento aparece duplicado no extrato', module: 'Pagamentos', priority: 'Alta', status: 'Aguardando cliente', assignee: 'Marina Costa', createdAt: '2026-08-01T13:40:00-03:00', updatedAt: '2026-08-03T09:22:00-03:00', slaHours: 4, slaState: 'ok', unreadClient: 1, description: 'O mesmo pagamento aparece duas vezes no extrato da unidade México. Solicitamos confirmação do identificador original.', attachments: ['extrato-mexico.pdf'], history: [{ text: 'Ticket criado por Camila Ferreira', time: '01 ago, 13:40' }, { text: 'Equipe solicitou o identificador do pagamento', time: 'Hoje, 09:22' }] },
  { id: 'TK-2077', company: 'Syngenta', platform: 'BIPPER', instance: 'syngenta-training', requester: 'Felipe Ramos', subject: 'Participantes sem certificado', module: 'Treinamento', priority: 'Média', status: 'Resolvido', assignee: 'Ana Souza', createdAt: '2026-07-31T14:30:00-03:00', updatedAt: '2026-08-02T16:15:00-03:00', slaHours: 8, slaState: 'ok', description: 'Participantes concluíram todas as aulas, mas o certificado não ficou disponível.', attachments: [], history: [{ text: 'Ticket criado por Felipe Ramos', time: '31 jul, 14:30' }, { text: 'Reprocessamento concluído', time: '02 ago, 16:10' }, { text: 'Ticket marcado como Resolvido', time: '02 ago, 16:15' }] },
  { id: 'TK-2076', company: 'BASF', platform: 'BIPPER', instance: 'basf-campanhas', requester: 'João Mendes', subject: 'Campanha antiga permanece no painel', module: 'Campanha', priority: 'Baixa', status: 'Fechado', assignee: 'Rafael Lima', createdAt: '2026-07-29T09:12:00-03:00', updatedAt: '2026-08-01T11:05:00-03:00', slaHours: 16, slaState: 'ok', description: 'Uma campanha encerrada ainda aparecia no painel inicial. O cache da instância foi atualizado.', attachments: ['painel-campanha.png'], history: [{ text: 'Ticket criado por João Mendes', time: '29 jul, 09:12' }, { text: 'Cache da instância atualizado', time: '01 ago, 10:40' }, { text: 'Ticket fechado após validação', time: '01 ago, 11:05' }] },
  { id: 'TK-2075', company: 'Zoetis', platform: 'BIPPER', instance: 'zoetis-ru-prod', requester: 'Camila Ferreira', subject: 'Treinamento não contabiliza presença', module: 'Treinamento', priority: 'Média', status: 'Aberto', assignee: 'Ana Souza', createdAt: '2026-08-02T14:18:00-03:00', updatedAt: '2026-08-03T12:30:00-03:00', slaHours: 8, slaState: 'ok', description: 'A lista de presença foi importada, mas três participantes continuam como ausentes.', attachments: ['lista-presenca.xlsx'], history: [{ text: 'Ticket criado por Camila Ferreira', time: 'Ontem, 14:18' }, { text: 'Atribuído para Ana Souza', time: 'Hoje, 12:30' }] },
  { id: 'TK-2074', company: 'BASF', platform: 'BIPPER', instance: 'basf-campanhas', requester: 'Juliana Alves', subject: 'Relatório de campanha não exporta em PDF', module: 'Campanha', priority: 'Baixa', status: 'Resolvido', assignee: 'Rafael Lima', createdAt: '2026-07-28T09:00:00-03:00', updatedAt: '2026-07-29T14:20:00-03:00', slaHours: 16, slaState: 'ok', description: 'A exportação do relatório de campanha em PDF trava na última etapa e não gera o arquivo.', attachments: [], history: [{ text: 'Ticket criado por Juliana Alves', time: '28 jul, 09:00' }, { text: 'Correção aplicada no gerador de PDF', time: '29 jul, 14:15' }, { text: 'Ticket marcado como Resolvido', time: '29 jul, 14:20' }] },
  { id: 'TK-2073', company: 'Elanco', platform: 'BIPPER', instance: 'elanco-br-prod', requester: 'Marcos Vieira', subject: 'Usuário não recebe convite de acesso', module: 'Plano de ação', priority: 'Média', status: 'Fechado', assignee: 'Bruno Tavares', createdAt: '2026-07-26T11:30:00-03:00', updatedAt: '2026-07-27T10:05:00-03:00', slaHours: 8, slaState: 'ok', description: 'Um novo usuário cadastrado não recebeu o e-mail de convite para acessar o plano de ação.', attachments: [], history: [{ text: 'Ticket criado por Marcos Vieira', time: '26 jul, 11:30' }, { text: 'Convite reenviado manualmente', time: '27 jul, 10:00' }, { text: 'Ticket fechado após confirmação de acesso', time: '27 jul, 10:05' }] },
  { id: 'TK-2072', company: 'Syngenta', platform: 'BIPPER', instance: 'syngenta-training', requester: 'Fernanda Dias', subject: 'Vídeo de treinamento não carrega no app', module: 'Treinamento', priority: 'Alta', status: 'Em andamento', assignee: 'Ana Souza', createdAt: '2026-08-01T13:10:00-03:00', updatedAt: '2026-08-02T16:40:00-03:00', slaHours: 4, slaState: 'ok', description: 'O vídeo do módulo 3 fica carregando indefinidamente no aplicativo mobile, mas funciona no navegador.', attachments: [], history: [{ text: 'Ticket criado por Fernanda Dias', time: '01 ago, 13:10' }, { text: 'Atribuído para Ana Souza', time: '01 ago, 13:30' }, { text: 'Identificado problema de encoding do vídeo', time: '02 ago, 16:35' }] },
  { id: 'TK-2071', company: 'Zoetis', platform: 'BIPPER', instance: 'zoetis-ca', requester: 'Camila Ferreira', subject: 'Pagamentos não geram nota fiscal automática', module: 'Pagamentos', priority: 'Alta', status: 'Aberto', assignee: 'Marina Costa', createdAt: '2026-08-02T09:45:00-03:00', updatedAt: '2026-08-02T15:20:00-03:00', slaHours: 4, slaState: 'ok', description: 'Pagamentos confirmados na instância não disparam a emissão automática de nota fiscal para o financeiro.', attachments: [], history: [{ text: 'Ticket criado por Camila Ferreira', time: '02 ago, 09:45' }, { text: 'Atribuído para Marina Costa', time: '02 ago, 10:02' }] },
  { id: 'TK-2070', company: 'BASF', platform: 'BIPPER', instance: 'basf-br-prod', requester: 'João Mendes', subject: 'Fechamento trimestral com atraso no processamento', module: 'Fechamento', priority: 'Crítica', status: 'Em andamento', assignee: 'Bruno Tavares', createdAt: '2026-08-01T08:15:00-03:00', updatedAt: '2026-08-03T11:50:00-03:00', slaHours: 1, slaState: 'risk', description: 'O fechamento trimestral está processando há mais de 12 horas sem concluir, travando o envio ao financeiro regional.', attachments: [], history: [{ text: 'Ticket crítico criado por João Mendes', time: '01 ago, 08:15' }, { text: 'Atribuído para Bruno Tavares', time: '01 ago, 08:40' }, { text: 'Reprocessamento do lote iniciado', time: '03 ago, 09:30' }] },
  { id: 'TK-2082', company: 'MasterCard', platform: 'Coaliza', instance: 'mastercard-br-prod', requester: 'Renata Prado', subject: 'Campanha não aparece no painel de resgates', module: 'Campanha', priority: 'Média', status: 'Aberto', assignee: 'Não atribuído', createdAt: '2026-08-04T09:30:00-03:00', updatedAt: '2026-08-04T09:30:00-03:00', slaHours: 8, slaState: 'ok', description: 'A campanha cadastrada não aparece listada no painel de resgates para os usuários finais.', attachments: [], history: [{ text: 'Ticket criado por Renata Prado', time: '04 ago, 09:30' }] }
];
...
const INITIAL_TIME_ENTRIES = {
  ...
  'TK-2074': [
    { id: 'TIME-2074-1', date: '2026-07-29', user: 'Rafael Lima', hours: 1, description: 'Correção do gerador de PDF' }
  ],
  'TK-2073': [
    { id: 'TIME-2073-1', date: '2026-07-27', user: 'Bruno Tavares', hours: 0.5, description: 'Reenvio de convite e validação de e-mail' }
  ],
  'TK-2072': [
    { id: 'TIME-2072-1', date: '2026-08-01', user: 'Ana Souza', hours: 2.5, description: 'Diagnóstico do player de vídeo no app' },
    { id: 'TIME-2072-2', date: '2026-08-02', user: 'Ana Souza', hours: 1.5, description: 'Ajuste de encoding do vídeo do módulo 3' }
  ],
  'TK-2071': [
    { id: 'TIME-2071-1', date: '2026-08-02', user: 'Marina Costa', hours: 4, description: 'Mapeamento da integração fiscal com o financeiro' }
  ],
  'TK-2070': [
    { id: 'TIME-2070-1', date: '2026-08-01', user: 'Bruno Tavares', hours: 3, description: 'Investigação do atraso no processamento' },
    { id: 'TIME-2070-2', date: '2026-08-03', user: 'Bruno Tavares', hours: 2.5, description: 'Reprocessamento do lote trimestral' }
  ]
};
const STORAGE_KEY = 'supportPrototypeTicketsV4';
...
const VIEW_MODE_KEY = 'bipper-support-view-mode';
function getViewMode() { return localStorage.getItem(VIEW_MODE_KEY) === 'card' ? 'card' : 'list' }
function setViewMode(mode) { localStorage.setItem(VIEW_MODE_KEY, mode) }
function initViewToggle(toggleEl, listEl, cardsEl) {
  const buttons = toggleEl.querySelectorAll('[data-view-choice]');
  const apply = mode => { buttons.forEach(b => { const active = b.dataset.viewChoice === mode; b.classList.toggle('active', active); b.setAttribute('aria-pressed', String(active)) }); listEl.hidden = mode === 'card'; cardsEl.hidden = mode !== 'card' };
  apply(getViewMode());
  buttons.forEach(b => b.addEventListener('click', () => { setViewMode(b.dataset.viewChoice); apply(b.dataset.viewChoice) }));
}
...
// Horas não existem em fração decimal (ex.: "2,93h") — sempre em horas e minutos reais, ex.: "2h56".
// Hora redonda não mostra minutos ("23h", não "23h00"); menos de 1h mostra só minutos ("30m", não "0h30").
function formatHours(value) {
  const totalMinutes = Math.round(Number(value || 0) * 60), h = Math.floor(totalMinutes / 60), m = totalMinutes % 60;
  if (!h) return `${m}m`;
  if (!m) return `${h}h`;
